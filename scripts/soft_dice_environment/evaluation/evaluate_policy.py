...
parser = argparse.ArgumentParser(description="Evaluate a soft-dice RSL-RL checkpoint.")
...
def main():
    ...
    env_cfg.scene.num_envs = min( int(args_cli.num_envs), int(args_cli.num_episodes))
    ...
    # Evaluation must always be finite.
    ...
    try:
        while (simulation_app.is_running() and len(records) < args_cli.num_episodes):
            ...
            if done_ids.numel() > 0:
                ...
                motion_finished = (termination_manager.get_term("motion_finished").detach().clone())

                hard_timeout = (termination_manager.get_term("time_out").detach().clone())
